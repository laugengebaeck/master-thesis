...
from util import ValidationRuleResult
        ...
        for node in topology.nodes:
            if topology.degree[node] >= 4:  # type: ignore
                return ValidationRuleResult(
                    False,
                    f"In the created topology, node {node} has more than 3 neighbors. That's wrong.",
                )
...
class NoCyclesValidation(TopologyValidationRule):
    def check(self, topology: nx.Graph, switch_positions: list[Vector2D]) -> ValidationRuleResult:
        ...
        if len(cycles) > 0:
            return ValidationRuleResult(
                False, f"The created topology contains cycles. That's wrong. Cycles: {cycles}"
            )
        return ValidationRuleResult(True)
